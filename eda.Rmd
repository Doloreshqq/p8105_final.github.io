...
ggplot(race_count, aes(x = reorder(race, -n), y = n)) +
...
```{r}
orientation_count = dass_new|>
  count(orientation)

ggplot(orientation_count, aes(x = reorder(orientation, -n), y = n)) +
  geom_bar(stat = 'identity', fill="steelblue") +
  labs(title = 'Distribution of Orientation', x = 'Orientation', y = 'Count') +
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
# Plot: Age Distribution for Each Gender
```{r}
boxplot(age~gender,
data=dass_new,
main="Age Distribution for Each Gender",
xlab="Gender",
ylab="Age",
col="pink",
border="steelblue"
