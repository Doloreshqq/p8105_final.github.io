...
country_coefficients <- data.frame(
  country = c("GB", "ID", "MY", "Other", "PH", "US"),
  coefficient = c(0.026293, -0.149925, -0.195490, -0.057339, 0.051250, -0.148052)
)
...
predictor_data <- model.matrix(~ country + education + urban + gender
                               + age + religion + orientation + married,
                               data = dass_fit_df)

# Compute the effects
effects <- allEffects(dass_fit_step, xlevels = list(predictor_data))

# Plot the effects
plot(effects, style = "level")
```
